    ...
    <Card className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-300">
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="bg-primary text-white text-sm px-2 py-1 rounded-md">{formatCurrency(tour.price)}</span>
        </div>
        <div className="mb-4">
        ...
        <Link href={`/tour/${tour.id}`}>
          <a className="block w-full bg-primary hover:bg-secondary text-white text-center py-2 rounded-md transition duration-200">Xem chi tiết</a>
        </Link>
      </CardContent>
    </Card>
  );
